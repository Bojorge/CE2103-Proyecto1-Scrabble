...
public: bool encontraryasignar(Lista* p){
        ...
        try {
            ...
            if (verif != true){
                ...
                int u= Jugadores->obtener_dato(0)->getPuntaje();
                u = u + puntaje;
                Jugadores->obtener_dato(0)->setPuntaje(u);
                std::cout<<"Putaje es : " << u <<std::endl;
            }
            //Asignar puntaje
        }catch(...){
            std::cout << " Error " << std::endl;
