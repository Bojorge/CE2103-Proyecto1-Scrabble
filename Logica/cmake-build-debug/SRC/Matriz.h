...
public:void poner_Dato(int x, int y, int dato){
        if(matriz[x][y] != 0) {
            return;
        }
        matriz[x][y] = dato;
...
public: void borrar_Dato(int x, int y){
        this->matriz[x][y] = 0;
    }

public: int retornar_Dato(int x , int y){
    return matriz[x][y];
}
...
public: void print_Matriz(ListaLetras* l){
        std::cout << " " << std::endl;
